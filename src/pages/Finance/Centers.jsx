...
const ManageContracts = () => {
  ...
  return (
    <div className="w-full space-y-6 sm:space-y-8 pt-4 sm:pt-6 animate-fade-in text-text-primary pb-8 sm:pb-10">
      ...
      <div className="grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
        {[
      ...
      <AnimatePresence>
        {open && (
          <motion.form
           ...
           className="bg-dark-800 p-6 rounded-3xl border border-dark-600/50 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"
          >
            {[
              ["name", "Center Name"],
              ["code", "Center Code"],
  ...
  {isLoading ? (
    ...
    centers.map((c, idx) => {
      ...
      return (
        <div
          key={c._id}
          className="bg-dark-800 border border-dark-600/50 rounded-2xl p-4 space-y-3"
        >
          <div className="flex justify-between items-start">
            <div>
              <p className="font-bold text-text-primary">
                {(page - 1) * entries + idx + 1}. {c.name}
              </p>
              <p className="text-xs text-text-muted">{c.code}</p>
            </div>
          ...
          <div className="text-sm text-text-muted space-y-1">
            <p>{c.email || "-"}</p>
            <p>{c.phone || "-"}</p>
            <p>Capacity: <span className="text-emerald-400 font-semibold">{c.capacity ?? 0}</span></p>
          </div>

          <div className="flex justify-end gap-4 pt-2 border-t border-dark-700">
            <button onClick={() => handleEdit(c)} className="text-primary text-sm">
              Edit
            </button>
            <button
      ...
      {/* ================= DESKTOP TABLE ================= */}
<div className="hidden md:block bg-dark-800 rounded-3xl border border-dark-600/50 shadow-md shadow-black/10">
  <div className="overflow-x-auto">
    <table className="w-full text-sm">
                  ...
                  <tr key={c._id} className="hover:bg-dark-700/40 transition">
                    ...
                    <td className="px-3 sm:px-4 py-3 text-text-muted whitespace-nowrap">
                      {typeof c.address === 'object' && c.address
                        ? `${c.address.street || ''}, ${c.address.city || ''}, ${c.address.state || ''} ${c.address.zipCode || ''}`.replace(/,\s*,/g, ',').trim()
                        : c.address || "-"}
                    </td>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-text-muted text-xs sm:text-sm">
  <span>
    Showing {centers.length ? (page - 1) * entries + 1 : 0} to{" "}
    {Math.min(page * entries, pagination.total || 0)} of{" "}
    {pagination.total || 0}
  </span>
  ...
  <div className="flex gap-2">
    <button
      ...
      disabled={page <= 1}
      className="p-2 rounded-lg border border-dark-700 disabled:opacity-40"
    >
